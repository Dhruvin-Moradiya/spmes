...
def faculty_list(request,list_id):
     ...
     sub_proj_id_list = sub_project.objects.filter(parent_project = list).values('id')
     is_reg = student_request.objects.filter(sid = request.session['sid']).filter(status=1).filter(sub_pid__in = sub_proj_id_list).exists()
     app_list = student_request.objects.filter( sid = request.session['sid']).filter(sub_pid__in = sub_proj_id_list)
     count=[]
     for i in fac_list:
...
def apply(request,p_id, sub_pid):
     ...
     if not student_request.objects.filter(sid = request.session['sid']).filter(sub_pid = sub_pid).exists():
          check = student_request.objects.filter(sid = request.session['sid']).filter(status = 0).exists()
...
def  submissions(request,p_id):
     ...
     grp_obj = grp_map.objects.filter(proj_id = p_id).filter(sid = request.session['sid']).first()
     if grp_obj is None:
          messages.info(request,"You have not assigned any group")
          return render(request,'student/submissions.html',{'p_id':p_id})
...
def certificate(request,p_id):
     ...
     try:
          ...
          messages.info(request,"You have not assiged any group")
          return render(request,"student/certificate.html",{'p_id':p_id})
...
def dowload_certificate(request,p_id):
     ...
     try:
        ...
        grp_size = grp_map.objects.filter(group_id= grp_map_obj.group_id).count()
         ...
         return HttpResponse("Certificate will be available at the end of project!!")
     student_submission_marks = submission.objects.filter(group_id = grp_map_obj.group_id)
