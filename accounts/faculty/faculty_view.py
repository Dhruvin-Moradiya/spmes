...
def student_groups(request,sp_id):
     ...
     accepted_student = obj.registred_student.all().values('sid','name')
     group_list = grp.objects.filter(sub_project_id =  sp_id)
     return render(request,"faculty/create_groups.html",{'group_list':group_list,'sp':sp_id,'acc_list':accepted_student})
...
def add_group(request,sp_id):
     ...
     obj = grp(sub_project_id = sub, student_id = ids, group_title = group_title)
     obj.save()
     ans = ""
     for id in sids:
          ans += id + " | "
...
def get_submission(request,sp_id):
     try:
          ...
          messages.warning(request,"No submission Found")
...
def exam(request, p_id):
     ...
     try:
          ...
          for obj in schedule_list:
               ...
               exam_list.append({'group_title':obj.group_id.group_title, 'faculty':obj.group_id.sub_project_id.related_faculty.name, 'sid':obj.group_id.student_id,'topic':obj.group_id.sub_project_id.sub_project_topic, 'submissions' : subm_obj,'marks':marks})
          print(exam_list)
          return render(request,"faculty/exam.html",{'exam_list':exam_list, 'p_id':p_id})
     except:
          messages.warning(request,"Not yet schedule")
          return redirect('faculty_project',p_id)
